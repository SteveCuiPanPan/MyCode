...
import com.suda.msgcenter.util.ImageCache.ImageCacheParams;
...
public enum ImageWorker {
			...
			public void onSetImage(final ImageView imageView, final Bitmap bitmap) {
				mHandler.post(new Runnable() {
					...
					public void run() {
						...
						AsyncDrawable aDrawable = (AsyncDrawable) imageView.getTag();
						if (aDrawable != null) {
							imageView.setBackgroundResource(aDrawable.getBgResID());
						}
					}
				});
			}
			...
			public void onError(final ImageView imageView) {
				mHandler.post(new Runnable() {
					...
					public void run() {
						if (imageView != null) {
							AsyncDrawable aDrawable = (AsyncDrawable) imageView.getTag();
							imageView.setImageResource(aDrawable != null ? aDrawable.getErrorResID() : errorResId);
						}
					}
				});
			}
	...
	public Bitmap loadBitmap(String path, int w, int h) {
		try {
			Bitmap bm = mImageCache.getBitmapFromMem(path);
			if (bm == null) {
				File file = downloadBitmap(path);
				bm = mImageCache.getBitmapFromDiskCache(file, w, h);
				if (bm != null) {
					mImageCache.addBitmapToCache(path, bm);
				}
			}
			return bm;
		} catch (Exception e) {
	...
	public void loadBitmapWithBg(final String path, final ImageView imageView, int bgresid) {
		loadBitmap(path, imageView, ScreenWeith / 3, ScreenWeith / 3, loadingResId, errorResId, bgresid);
	}

	public void loadBitmapWithBg(final String path, final ImageView imageView, int w, int h, int bgresid) {
		loadBitmap(path, imageView, w, h, loadingResId, errorResId, bgresid);
	}

	public void loadBitmapWithBg(final String path, final ImageView imageView, int w, int h, int loadingRes, int bgresid) {
		loadBitmap(path, imageView, w, h, loadingRes, errorResId, bgresid);
	}
	...
	public void loadBitmap(final String path, final ImageView imageView, int w, int h, int loadingRes) {
		loadBitmap(path, imageView, w, h, loadingRes, errorResId, bgResId);
	}

	public void loadBitmap(final String path, final ImageView imageView, int loadingRes) {
		loadBitmap(path, imageView, ScreenWeith / 3, ScreenWeith / 3, loadingRes, errorResId, bgResId);
	}

	public void loadBitmap(final String path, final ImageView imageView, int w, int h, int loadingRes, int errRes,
			int bgRes) {
		...
		if (result != null && !result.isRecycled()) {
			...
			final SearchTask task = new SearchTask(path, imageView, w, h, mIHandleCache);
			final AsyncDrawable asyncDrawable = new AsyncDrawable(loadingRes, errRes, bgRes, task);
...
//		}
	...
	public class SearchTask implements Runnable {
		...
		public SearchTask(final String path, final ImageView imageView, int w, int h,
		...
		public void run() {
			...
			if (bitmap == null && mImageCache != null && !stop && getAttachedImageView() != null && onScreen) {
				try {
					File file = downloadBitmap(path);
					bitmap = mImageCache.getBitmapFromDiskCache(file, reqW, reqH);
					if (bitmap == null) {
	...
	public class AsyncDrawable {
		...
		public AsyncDrawable(int loadingRes, int errResId, int bgResid, SearchTask searchTask) {
	...
	private File downloadBitmap(String urlString) throws IOException {
		...
		try {
			...
			InputStream in = new BufferedInputStream(urlConnection.getInputStream(), IO_BUFFER_SIZE);
